...
- Use hosted OpenAI extraction to build both a topic graph and a self-model layer with patterns, principles, preferences, decisions, and agent instructions.
...
- Route lower-confidence inferred self-model signals into `Context Graph/Review Queue/` so users can approve or reject them in Markdown before they become canonical.
...
- Generate a sectioned `Context Graph/Agent Context/` pack optimized for LLM retrieval, with identity and agent instructions before projects, topics, entities, and artifacts.
...
The importer only writes inside the configured output folder, defaulting to `Context Graph`. Existing files are updated only when they are marked with `pcg_managed: true`. Review queue notes are protected so status edits such as `approved` and `rejected` survive future imports.
